...
let DepartmentListComponent = class DepartmentListComponent {
    ...
    openPDF() {
        ...
        let doc = new jspdf__WEBPACK_IMPORTED_MODULE_5__('p', 'pt', 'a4');
...
let EmployeeAttendanceListComponent = class EmployeeAttendanceListComponent {
    ...
    openPDF() {
        let DATA = this.pdfTable.nativeElement;
        let doc = new jspdf__WEBPACK_IMPORTED_MODULE_6__('p', 'pt', 'a4');
        doc.fromHTML(DATA.innerHTML, 15, 15);
        doc.output('dataurlnewwindow');
    }
    downloadPDF() {
...
let EmployeeComponent = class EmployeeComponent {
    constructor(fb, dialog, _empService, data) {
        this.fb = fb;
        this.dialog = dialog;
...
let EmployeeService = class EmployeeService {
    ...
    addOrUpdateEmployee(isEmp) {
        const existEmp = this.getEmployeeId(isEmp.EmployeeId);
        if (existEmp === -1) {
            //will creates leave
...
let HrmLogoutComponent = class HrmLogoutComponent {
    constructor(router, authService, dialogRef) {
        this.router = router;
        this.authService = authService;
...
HrmLogoutComponent.ctorParameters = () => [
...
let LeaveListComponent = class LeaveListComponent {
    constructor(fb, dialog, _leaveService) {
        this.fb = fb;
        this.dialog = dialog;
...
let LeaveComponent = class LeaveComponent {
    constructor(fb, dialog, _leaveService, data) {
        this.fb = fb;
        this.dialog = dialog;
        this._leaveService = _leaveService;
        this.data = data;
...
let LeaveService = class LeaveService {
    ...
    addOrUpdateLeave(leave) {
        const existLeave = this.getLeaveId(leave.LeaveId);
        if (existLeave === -1) {
            //will creates leave
    ...
    deleteLeave(id) {
        const delLeave = this.getLeaveId(id);
...
let LoanComponent = class LoanComponent {
    constructor(fb, dialog, _loanService, data) {
        this.fb = fb;
        this.dialog = dialog;
        this._loanService = _loanService;
        this.data = data;
...
let LoanService = class LoanService {
    ...
    addOrUpdateLoan(loan) {
        const existLoan = this.getLoanId(loan.LoanId);
        if (existLoan === -1) {
            //will creates leave
...
let NavbarComponent = class NavbarComponent {
    ...
    openLogout() {
        ...
        this.isPopupOpened = true;
        const dialogRef = this.dialog.open(_hrm_login_hrm_logout_hrm_logout_component__WEBPACK_IMPORTED_MODULE_5__["HrmLogoutComponent"], {
            data: {}
        });
        dialogRef.afterClosed().subscribe(result => {
        });
...
let PayrollComponent = class PayrollComponent {
    constructor(fb, dialog, _payService, data) {
        this.fb = fb;
        this.dialog = dialog;
...
let PayrollService = class PayrollService {
    ...
    addOrUpdatePayroll(payroll) {
        const existPay = this.getPayrollId(payroll.PayrollId);
        if (existPay === -1) {
            //will creates leave
...
let TrainingService = class TrainingService {
    constructor(_httpClientService) {
    ...
    addOrUpdateTraining(train) {
        const existTrain = this.getTrainId(train.TrainingId);
        if (existTrain === -1) {
            //will creates leave
...
let TrainingListComponent = class TrainingListComponent {
    constructor(dialog, _trainService) {
        this.dialog = dialog;
...
let TrainingComponent = class TrainingComponent {
    constructor(fb, dialog, _trainService, data) {
        this.fb = fb;
        this.dialog = dialog;
        this._trainService = _trainService;
        this.data = data;
...
let UserService = class UserService {
    ...
    addOrUpdateUser(addUser) {
        const users = this.getUserId(addUser.UserId);
        if (users === -1) {
            //will creates leave
            addUser.UserId = this._userDetail.length + 1;
...
let UserComponent = class UserComponent {
    constructor(fb, dialog, _userService, data) {
        this.fb = fb;
        this.dialog = dialog;
        this._userService = _userService;
        this.data = data;
